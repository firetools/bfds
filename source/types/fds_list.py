...
class FDSList(list):
    ...
    def __init__(
        ...
        if any((f90_namelists, f90_params, f90_value)):
            self.from_fds(
                f90_namelists=f90_namelists, f90_params=f90_params, f90_value=f90_value
            )
    ...
    def get_fds_namelists(self, fds_label=None, remove=False):
        ...
        for i, item in enumerate(self):
            match item:
                ...
                case FDSList():
                    items.extend(
                        item.get_fds_namelists(fds_label=fds_label, remove=remove)
                    )
        if remove:
    ...
    def get_fds_params(self, fds_label=None, remove=False):
        ...
        for i, item in enumerate(self):
            match item:
                ...
                case FDSList():
                    items.extend(
                        item.get_fds_params(fds_label=fds_label, remove=remove)
                    )
        if remove:
            for i in reversed(indexes):
                self.pop(i)
            ...
            match item:
                ...
                case FDSList():
                    ...
                    if multi_ps_new:
                        if multi_ps:
                            raise ValueError(
                                "Only one FDSMulti in FDSNamelist (from child)."
                            )
    ...
    _RE_SCAN_DECIMAL_POS = re.compile(
        r"\.([0-9]+)", re.VERBOSE | re.DOTALL | re.IGNORECASE
    )

    # scan integer postions of exp notation
    _RE_SCAN_INTEGER = re.compile(
        r"([0-9]*)\.?[0-9]*[eE]", re.VERBOSE | re.DOTALL | re.IGNORECASE
    )
...
class FDSNamelist(FDSList):
    ...
    def __init__(
        self, fds_label, iterable=(), f90_params=None, msgs=(), msg=None
    ) -> None:
        """!
        Class constructor.
        ...
        for p in n:
            ...
            if not fds_values:
                ...
                if len(word) <= MAXLEN - INDENT or len(fds_values) == 1:
                    ...
                    body = append_word(
                        body, word=f"{fds_label}={fds_values[0]},"
                    )  # first
                    for v in fds_values[1:-1]:
                        body = append_word(body, word=f"{v},", separator="")
                    body = append_word(
                        body, word=f"{fds_values[-1]}", separator=""
                    )  # last
        body[-1] += " /"  # close
        return "\n".join(body)

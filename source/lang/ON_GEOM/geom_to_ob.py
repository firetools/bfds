...
"""!
BFDS, translate FDS GEOM notation to Blender object geometry.
"""
...
def geom_to_ob(
    ...
    if filepath:
        ...
        _, fds_verts, fds_faces, fds_surfs, _, geom_type = bingeom.read_bingeom_file(
            filepath
        )
        fds_faces_surfs = None
    geom_to_mesh(
...
def geom_to_mesh(
    ...
    if max(fds_surfs) > len(me.materials):
        raise BFException(
            me,
            f"Bad GEOM: Max FACE SURF index ({max(fds_surfs)}) > SURF_ID {len(me.materials)}",
        )
    # Create a new bmesh,
    bm = bmesh.new()
